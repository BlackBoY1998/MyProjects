...
using WebTimeSheetManagement.Controllers;
...
namespace CRMApplication.Controllers
{
    ...
    {
        private ITimeSheetExport _ITimeSheetExport;
        private ITimeSheet _ITimeSheet;
        private TimeSheetExportConsume ObjTimeSheet;
        public TimeSheetExportController()
        {
            ObjTimeSheet = new TimeSheetExportConsume();
        ...
        public ActionResult TimeSheetReport(TimeSheetExportUserModel objtimesheet)
        {
            ...
            try
            {
                ...
                Task<string> filename = ObjTimeSheet.GetUsernamebyRegistrationID("GetUsernamebyRegistrationID", objtimesheet.RegistrationID);
                    ...
                    if (timesheetdata.Tables.Count > 0)
                    {
                        ...
                        else
                        {
                            ...
                            for (int k = 0; k < timesheetdata.Tables[0].Rows.Count; k++)
                            {
                                ...
                                var data = ObjTimeSheet.GetPeriodsbyTimeSheetMasterID("GetPeriodsbyTimeSheetMasterID", Convert.ToInt32(timesheetID));
                                ...
                                var ListofProjectNames = ObjTimeSheet.GetProjectNamesbyTimeSheetMasterID("GetProjectNamesbyTimeSheetMasterID", Convert.ToInt32(timesheetID));
                                ...
                                for (int i = 0; i < ListofProjectNames.Count(); i++)
                                {
                                    ...
                                    var ListofDescription = CRMApplication.Helpers.MethodonViews.GetDescriptionbyTimeSheetMasterID(timesheetID, ListofProjectNames[i].ProjectID);
                                    DataRow row1 = dt.NewRow();
                            ...
                            Response.ClearContent();
                            Response.Buffer = true;
                            Response.AddHeader("content-disposition", "attachment; filename=" + filename.Result.Trim() + ".xls" + "");
                            Response.ContentType = "application/ms-excel";
                            Response.Charset = "";
                            StringWriter objStringWriter = new StringWriter();
                            HtmlTextWriter objHtmlTextWriter = new HtmlTextWriter(objStringWriter);
        ...
        public JsonResult ListofEmployees()
        {
            try
            {
                var ListofEmployees = ObjTimeSheet.ListofEmployees("ListofEmployees", Convert.ToInt32(Session["AdminUser"]));
